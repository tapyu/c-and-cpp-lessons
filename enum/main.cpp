...
int main() {
    ...
    int istatus         = status; // `enum` values can be used without explicit casting in most contexts
    int iselected_color = static_cast<int>(selected_color); // `enum class` require explicit casting when used in contexts where an integral value is expected
